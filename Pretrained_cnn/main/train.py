...
from data_loader import LeafDataset
...
parser.add_argument('--feature-path', type=str, default='saved_feature', help='Feature Extractor path')
...
if __name__ == '__main__':
    ...
    # Load datasets
    train_set = LeafDataset(base_path, train_txt, image_size=(128, 128), mode='train')
    validate_set = LeafDataset(base_path, validate_txt, image_size=(128, 128), mode='validate')
    ...
    # Set up model
    model = CNN(num_classes=2, dropout_rate=0.1)
    model.to(device)  
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.features.state_dict(), os.path.join(args.feature_path, f'feature_extractor_best_epoch_{epoch+1}.pth'))
    ...
    plt.title('Training and Validation Loss of Pretrained CNN')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(args.logs_path, 'loss_curve_pretrained_cnn.pdf'))
    plt.show()
